...
#[derive(Debug)]
pub struct CBuf(*const i8, usize);
...
impl<T> DatabaseRef<T> {
    ...
    pub fn serialize(&self) -> Result<CBuf, Error> {
        let mut ptr = ptr::null_mut();
        let mut size: usize = 0;

        unsafe { ffi::hs_serialize_database(self.as_ptr(), &mut ptr, &mut size).map(|_| CBuf(ptr as *mut _, size)) }
    }
